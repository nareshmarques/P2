...
"   -1 FLOAT, --alfa_1=FLOAT         Margin for k1 [default: 2.9]\n"
"   -2 FLOAT, --alfa_2=FLOAT         Margin for k2 [default: 6.025]\n"
"   -s INT, --count_ms=INT           Maybe Silence (MS) frame counter [default: 8]\n"
"   -m INT, --count_mv=INT           Maybe Voice (MV) frame counter [default: 13]\n"
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        } else if (!strcmp(option->olong, "--alfa_1")) {
            if (option->argument)
                args->alfa_1 = option->argument;
        } else if (!strcmp(option->olong, "--alfa_2")) {
            if (option->argument)
                args->alfa_2 = option->argument;
        } else if (!strcmp(option->olong, "--count_ms")) {
            if (option->argument)
                args->count_ms = option->argument;
        } else if (!strcmp(option->olong, "--count_mv")) {
            if (option->argument)
                args->count_mv = option->argument;
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    DocoptArgs args = {
        0, 0, 0, (char*) "2.9", (char*) "6.025", (char*) "8", (char*) "13",
        NULL, NULL, NULL,
